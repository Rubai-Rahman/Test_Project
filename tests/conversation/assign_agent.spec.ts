...
test("assign_agnet Test ", async ({ page }) => {
  ...
  await page
    .locator("div")
    .filter({ hasText: /^#4Thawab A\/test team 2You$/ })
    .locator("svg")
    .click();
  await page
    .locator("#react-select-2-option-1-0")
    .getByText("Thawab AYou")
    .click();
  await expect(
    page.getByText("You assigned this conversation to yourself")
  ).toHaveText("You assigned this conversation to yourself");
});
